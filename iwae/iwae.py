...
class IWAE:
    def __init__(self, q_layers, p_layers, prior, alpha = 0.0):
        ...
        alpha = min(1.0, alpha)
        self.alpha = alpha
        print 'initialised AAE with alpha =', self.alpha
    ...
    def gradIminibatch_srng(self, x, srng, num_samples, model_type='iwae', backward_pass='full'):
        ...
        if backward_pass == 'max':
            ...
            print "Training an AAE with alpha = %.2f, k = %d" % (self.alpha, num_samples)
            return collections.OrderedDict([(
                                             param,
                                             theano.clone(
    ...
    @staticmethod
    def random(latent_units, hidden_units_q, hidden_units_p, bias=None, data_type='binary', alpha = 0.0):
        layers_q = []
        ...
        return IWAE(layers_q, layers_p, prior, alpha)


def random_iwae(latent_units, hidden_units_q, hidden_units_p, dataset, alpha = 0.0):
    return IWAE.random(latent_units, hidden_units_q, hidden_units_p,
                       bias=dataset.get_train_bias_np(), alpha = alpha)
...
def get_samples(model, num_samples, seed=123):
    ...
    # for Frey Face shape = (20, 28)
    # for MNIST shape = (28, 28)
    return reshape_and_tile_images(samples_function(), shape=(28, 28))
...
def get_first_q_layer_weights(model):
    shape = (28, 28)
    return utils.reshape_and_tile_images(model.first_q_layer_weights_np().T, shape=shape)

def get_last_p_layer_weights(model):
    shape = (28, 28)
    return utils.reshape_and_tile_images(model.last_p_layer_weights_np(), shape=shape)
