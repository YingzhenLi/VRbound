...
class VA:
    ...
    def _computeGradients(self, x, K = 50):
        ...
        # then compute the weights
        log_ws_matrix = lowerbound_x.reshape(K, N)
        # now compute the VR-max
        #max_ind = np.argmax(np.asarray(log_ws_matrix), axis = 0)
        max_ind = np.argmax(log_ws_matrix, axis = 0)
        lowerbound = np.sum(np.max(log_ws_matrix, axis = 0))
        # now get the max particle
        max_ind = list(max_ind + np.arange(N) * K)         
        x = x[:, max_ind]
        h_encoder = h_encoder[:, max_ind]
        mu_encoder = mu_encoder[:, max_ind]
        log_sigma_encoder = log_sigma_encoder[:, max_ind]
        eps = eps[:, max_ind]
        z = z[:, max_ind]
        h_decoder = h_decoder[:, max_ind]
        y = y[:, max_ind]
        if self.continuous:
            log_sigma_decoder = log_sigma_decoder[:, max_ind]
