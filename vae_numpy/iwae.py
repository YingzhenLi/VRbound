...
class VA:
    ...
    def _computeGradients(self, x, K = 50):
        ...
        # now compute the IWAE bound
        lowerbound = np.sum(logsumexp(log_ws_matrix, axis = 0) - np.log(K))
        log_ws_minus_max = log_ws_matrix - np.max(log_ws_matrix, axis=0)
        ws = np.exp(log_ws_minus_max)
        ws = ws / np.sum(ws, axis=0)
        ws_vec = ws.T.reshape(-1)
        ...
        # W5
        dy_dSig = (y * (1 - y))
        dp_dW5 = (dp_dy * dy_dSig * ws_vec).dot(h_decoder.T)
        dp_db5 = np.sum(dp_dy * dy_dSig * ws_vec, axis=1)[:, np.newaxis]

        if self.continuous:
            # W6
            dp_dW6 = (dp_dlogsigd * dy_dSig * ws_vec).dot(0.5 * h_decoder.T)
            dp_db6 = np.sum(dp_dlogsigd * dy_dSig * ws_vec, axis=1)[:, np.newaxis]
        ...
        # W4
        dp_dW4 = (dp_dHd * dHd_df * ws_vec).dot(z.T)
        dp_db4 = np.sum(dp_dHd * dHd_df * ws_vec, axis=1)[:, np.newaxis]
        ...
        dp_dW2 = (dp_dmue * ws_vec).dot(dmue_dW2.T)
        dp_db2 = dp_dmue * ws_vec

        dKLD_dmue = -mu_encoder
        dKLD_dW2 = (dKLD_dmue * ws_vec).dot(dmue_dW2.T)
        dKLD_db2 = dKLD_dmue * ws_vec
        ...
        dp_dW3 = (dp_dlogsige * ws_vec).dot(dlogsige_dW3.T)
        dp_db3 = dp_dlogsige * dlogsige_db3 * ws_vec

        dKLD_dlogsige = 1 - np.exp(2 * log_sigma_encoder)
        dKLD_dW3 = (dKLD_dlogsige * ws_vec).dot(dlogsige_dW3.T)
        dKLD_db3 = dKLD_dlogsige * dlogsige_db3 * ws_vec
        ...
        dp_dW1_1 = (dp_dtanh * ws_vec).dot(dtanh_dW1.T)
        dp_db1_1 = dp_dtanh * ws_vec
        ...
        dp_dW1_2 = (dp_dtanh_2 * ws_vec).dot(dtanh_dW1.T)
        dp_db1_2 = dp_dtanh_2 * ws_vec
        ...
        dKLD_dW1_1 = (dKLD_dtanh * ws_vec).dot(dtanh_dW1.T)
        dKLD_db1_1 = dKLD_dtanh * ws_vec

        dKLD_dtanh_2 = dKLD_dHe_2.T * dHe_df
        dKLD_dW1_2 = (dKLD_dtanh_2 * ws_vec).dot(dtanh_dW1.T)
        dKLD_db1_2 = dKLD_dtanh_2 * ws_vec
